...
async function reportReply(req, res) {
  try {
    ...
    if (!reported.length) return res.status(404).send("Reply cannot found");
...
async function deleteReply(req, res) {
  try {
    ...
    if (deleted[0].delete_password !== delete_password) return res.status(400).send("incorrect password");
    const updatedReplies = data.replies.map(reply => {
